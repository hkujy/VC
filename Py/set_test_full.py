...
def one_test(mp: mypara.MyParaClass, _name="try"):
    ...
    # elif mp.para_dict["Procedure"] =="RecoverOne":
        ...
        print("Warning: File: set_test_full_func_one_test,on the procedure parameter setting")
...
def test_recover_case(mp: mypara.MyParaClass, _disrupt_links, _method):
    """
        test recover case
    """
    # step 0 check the input
    if _method is not "Eval_base_and_given_net" or _method is not "Eval_remove_each_one":
        print("Warning: wrong input _method for the recover")
    # step 1: test the number
    mp.para_dict["Procedure"] = _method
    
    if _method is "Eval_remove_each_one":
        test_recover_case_step1_remove(mp,_num_links)
    elif _method is "Eval_base_and_given_net":
        cases = one_test(
            mp, _name=mp.para_dict["Network"]+"_recover_"+str(_num_links)+"_links")
