...
//********//
// add 13 Oct 2019 by Jy

void StarNetwork::calculateTotalCost() const{
	TotalCost = 0.0;
	for (StarLink *link = beginOnlyLink(); link != NULL; link = getNextOnlyLink()) {
		//std::cout << "flow = " << link->getFlow() << ", time = " << link->getTime() << std::endl;
		TotalCost += link->getTime()*link->getFlow();
	}
}
